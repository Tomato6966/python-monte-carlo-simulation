"""
Monte Carlo Simulator for a Core-Satellite portfolio consisting of MSCI World (Core) 
and Amundi Leveraged MSCI USA (Satellite). Allows simulation of different 
portfolio weightings, durations, and savings rates.

Version 3.0 (UI)

Supports optional rebalancing:
- "none" (default): no rebalancing
- "annual": annually on the 252nd day
- "dynamic:<max_satellite_weight>": when satellite exceeds max weight

History:
- 2.0: Initial version
- 2.1: Daily returns optionally from normal or t-distribution (fat tails).
- 2.2: Seed optionally configurable via CLI for fully random simulations.
- 2.3: Output of IRR (internal rate of return) per percentile for savings plans
- 2.4: Correlation between Core and Satellite assets adjustable (default: 0.96)
- 2.5: Realistic leveraged ETF modeling: the Satellite ETF is simulated as a 3x leveraged index path
- 3.0: No CLI config needed anymore, fully usable via UI, data retrieved via Yahoo Finance, CERT issues ignored. 
       Output graph is interactive (movable and zoomable). Output data enhanced with Max Drawdown & Recovery Time (@tomato6966 (github) / @chrissy8283 (discord))
- 3.1: Added detailed function descriptions and comments for crucial steps

New Pip Install: pip install numpy matplotlib pandas scipy requests
Important: Install Python with tkinter: sudo apt-get install python-tk
"""
...
def get_historical_data(portfolio_config: List[Dict], period: str = "10y") -> Tuple[pd.Series, pd.DataFrame]:
    ...
    print(f"Loading historical data for {tickers} for the period '{period}'...")
    ...
    for ticker in tickers:
        ...
        if not series.empty:
            ...
            print(f"Warning: No data received for {ticker}. This ticker will be ignored.")

    if not all_tickers_data:
        raise ValueError("No data received for any ticker from Yahoo Finance. Please check tickers or adjust the period.")
    ...
    if data.empty:
        raise ValueError("No complete data available after cleaning. Please check tickers.")

    # Calculating daily returns and annual metrics
    daily_returns = data.pct_change().dropna()

    if daily_returns.empty:
        raise ValueError("Not enough data points after calculating daily returns. Please check tickers or extend period.")
...
def _run_single_simulation(params_tuple):
    ...
    for day in range(1, total_days):
        ...
        if dist == "t":
            ...
            raise ValueError("Unknown distribution model. Allowed: 'normal', 't'")
...
def simulate_portfolio(
    ...
    try:
        ...
        if cancel_event and cancel_event.is_set():
            print("Simulation was canceled by user action.")
        else:
            raise e
    finally:
        if pool is not None:
...
class PortfolioSimulatorGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(input_frame, text="Initial Capital (EUR):").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Monthly Savings Rate (EUR):").grid(row=1, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Annual Savings Rate Increase (%):").grid(row=2, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Investment Duration (Years):").grid(row=3, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Rebalancing Strategy:").grid(row=4, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Distribution:").grid(row=5, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Degrees of Freedom (t-Distribution):").grid(row=6, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Random Seed (optional):").grid(row=7, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Historical Period (e.g., 10y, max):").grid(row=8, column=0, padx=5, pady=2, sticky="w")
        ...
        ttk.Label(input_frame, text="Number of Simulations:").grid(row=9, column=0, padx=5, pady=2, sticky="w")
        ...
        # Ticker search frame
        search_frame = ttk.LabelFrame(self.master, text="Ticker Search (Yahoo Finance)", padding=(20, 10))
        search_frame.pack(padx=10, pady=10, fill="x", expand=False)
        ttk.Label(search_frame, text="Search Term (Name/ISIN/Symbol):").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        self.search_query_entry = ttk.Entry(search_frame)
        self.search_query_entry.grid(row=0, column=1, padx=5, pady=2, sticky="ew")
        self.search_button = ttk.Button(search_frame, text="Search", command=self.perform_ticker_search)
        self.search_button.grid(row=0, column=2, padx=5, pady=2)
        self.search_results_tree = ttk.Treeview(search_frame, columns=("Symbol", "Name", "Exchange"), show="headings", height=5)
        self.search_results_tree.heading("Symbol", text="Symbol")
        self.search_results_tree.heading("Name", text="Name")
        self.search_results_tree.heading("Exchange", text="Exchange")
        self.search_results_tree.column("Symbol", width=100, anchor="w")
        self.search_results_tree.column("Name", width=250, anchor="w")
        self.search_results_tree.column("Exchange", width=80, anchor="w")
        ...
        portfolio_frame = ttk.LabelFrame(self.master, text="Portfolio Configuration", padding=(20, 10))
        ...
        ttk.Label(self.portfolio_scrollable_frame, text="Weight (%)", font=('Arial', 10, 'bold')).grid(row=0, column=1, padx=5, pady=2)
        button_frame = ttk.Frame(portfolio_frame)
        button_frame.pack(pady=5)
        ttk.Button(button_frame, text="Add Ticker", command=self.add_portfolio_row).pack(side="left", padx=5)
        ttk.Button(button_frame, text="Remove Ticker", command=self.remove_portfolio_row).pack(side="left", padx=5)

        # Action buttons frame
        action_button_frame = ttk.Frame(self.master)
        action_button_frame.pack(pady=10)
        self.cancel_button = ttk.Button(action_button_frame, text="Cancel", command=self.cancel_simulation, state="disabled", style='Cancel.TButton')
        self.cancel_button.pack(side="left", padx=10)
        self.calculate_button = ttk.Button(action_button_frame, text="Start Simulation", command=self.start_simulation_thread)
        self.calculate_button.pack(side="left", padx=10)
        self.status_label = ttk.Label(self.master, text="Ready", font=('Arial', 10, 'italic'), foreground='blue')
        self.status_label.pack(pady=5)

        # Yahoo Finance link
        yahoo_link_frame = ttk.Frame(self.master)
        yahoo_link_frame.pack(pady=5)
        ttk.Label(yahoo_link_frame, text="Find more tickers at: ").pack(side="left")
    ...
    def remove_portfolio_row(self):
        ...
        if len(self.portfolio_entries) > 1:
            ...
            messagebox.showwarning("Warning", "At least one ticker must be present.")
    ...
    def get_portfolio_config_from_ui(self) -> List[Dict]:
        ...
        for ticker_entry, weight_entry in self.portfolio_entries:
            ...
            try:
                ...
                raise ValueError(f"Invalid weight for ticker '{ticker}': {e}")
        if not portfolio_config:
            raise ValueError("Please add at least one ticker and weight.")
        total_weight = sum(item['weight'] for item in portfolio_config)
        if not np.isclose(total_weight, 1.0):
            response = messagebox.askyesno(
                "Weight Warning",
                f"The sum of weights ({total_weight*100:.2f}%) is not 100%. Would you like to normalize the weights automatically?",
                icon='warning'
            )
            if response:
                ...
                raise ValueError("The sum of weights must equal 100% or be normalized.")
    ...
    def perform_ticker_search(self):
        ...
        if not search_query:
            ...
            return
        self._update_status_label(f"Searching for '{search_query}'...", "orange")
    ...
    def _run_ticker_search_in_thread(self, query: str):
        ...
        try:
            ...
            self.master.after(0, messagebox.showerror, "Search Error", f"An error occurred during ticker search: {e}")
            self._update_status_label("Search failed", "red")
        finally:
            self.master.after(0, self._update_status_label, "Ready", "blue")
    ...
    def _display_search_results(self, results: List[Dict]):
        ...
        for item in self.search_results_tree.get_children():
            self.search_results_tree.delete(item)
        if not results:
            self.search_results_tree.insert("", "end", values=("", "No results found", ""))
            return
    ...
    def on_search_result_double_click(self, event):
        ...
        if selected_item:
            ...
            self._update_status_label(f"'{ticker_symbol}' added to portfolio.", "green")

    def cancel_simulation(self):
        """Cancels an ongoing simulation and cleans up resources."""
        self._update_status_label("Canceling simulation...", "red")
        ...
        if self.simulation_pool_ref:
            ...
            try:
                ...
                pool.close()
                pool.join()
                print("Multiprocessing pool terminated and closed.")
            except Exception as e:
                print(f"Error terminating pool: {e}")
            finally:
                self.simulation_pool_ref.clear()
        self._update_status_label("Simulation canceled. Ready for new start.", "blue")
    ...
    def start_simulation_thread(self):
        ...
        self._update_status_label("Starting simulation...", 'blue')
    ...
    def _run_simulation_in_thread(self):
        ...
        try:
            ...
            if self.cancel_event.is_set():
                self._update_status_label("Simulation canceled before starting.", "red")
                return

            # Step 2: Getting portfolio configuration
            portfolio_config = self.get_portfolio_config_from_ui()

            self._update_status_label("Loading historical data...", "orange")
            if self.cancel_event.is_set():
                self._update_status_label("Simulation canceled during data loading.", "red")
                return
            ...
            if self.cancel_event.is_set():
                self._update_status_label("Simulation canceled after data loading.", "red")
                return

            self._update_status_label("Portfolio defined and data prepared...", "orange")
            ...
            if not filtered_portfolio_config:
                raise ValueError("No valid tickers with data available for simulation.")
            ...
            def simulation_progress_callback(current: int, total: int):
                if not self.cancel_event.is_set():
                    self._update_status_label(f"Running {total:,} simulations: {current:,}/{total:,}", "orange")

            self._update_status_label(f"Running {n_simulations:,} simulations: 0/{n_simulations:,}", "orange")

            if self.cancel_event.is_set():
                self._update_status_label("Simulation canceled before simulation start.", "red")
                return
            ...
            if self.cancel_event.is_set():
                is_cancelled = True
                self._update_status_label("Simulation canceled.", "red")
                return

            self._update_status_label("Displaying results window...", "green")
            ...
            if not self.cancel_event.is_set():
                self.master.after(0, messagebox.showerror, "Input Error", str(e))
                self._update_status_label("Error: " + str(e), "red")
        except Exception as e:
            if not self.cancel_event.is_set():
                self.master.after(0, messagebox.showerror, "Simulation Error", f"An unexpected error occurred: {e}")
                self._update_status_label("Error occurred", "red")
                import traceback
                traceback.print_exc()
        finally:
            ...
            if not is_cancelled:
                self._update_status_label("Ready", "blue")
            self.simulation_pool_ref.clear()
    ...
    def show_results_window(self, results: np.ndarray, invested: float, irr_values: np.ndarray, max_drawdown_percents: np.ndarray, recovery_times_in_years: np.ndarray, years: int, final_portfolio_config: List[Dict]):
        ...
        portfolio_display_frame = ttk.LabelFrame(results_window, text="Used Portfolio Configuration", padding=(10, 5))
        ...
        summary_frame = ttk.LabelFrame(results_window, text="Summary of Final Values", padding=(10, 5))
        ...
        for i, p in enumerate(percentiles):
            ...
            summary_data.append([
                ...
                f"{max_dd * 100:.2f}%", f"{rec_time:.2f} Years" if not np.isinf(rec_time) else "N/A"
            ])
        tree_columns = ("Percentile", "Final Value (EUR)", "CAGR (%)", "Total Return (%)", "IRR (%)", "Max Drawdown (%)", "Max Recovery Time (Years)")
        tree = ttk.Treeview(summary_frame, columns=tree_columns, show="headings")
        for col in tree_columns:
            tree.heading(col, text=col)
            if "Percentile" in col: tree.column(col, width=80, anchor="center")
            elif "Final Value" in col: tree.column(col, width=120, anchor="e")
            elif "CAGR" in col or "IRR" in col or "Total Return" in col or "Max Drawdown" in col: tree.column(col, width=100, anchor="e")
            elif "Recovery Time" in col: tree.column(col, width=120, anchor="e")
        for row in summary_data:
            tree.insert("", "end", values=row)
        tree.pack(fill="both", expand=True)

        # Plotting histogram of final values
        plot_frame = ttk.LabelFrame(results_window, text="Distribution of Final Values", padding=(10, 5))
        plot_frame.pack(padx=10, pady=10, fill="both", expand=True)
        fig = Figure(figsize=(10, 6), dpi=100)
        ax = fig.add_subplot(111)
        ax.hist(results, bins=100, alpha=0.75, color='royalblue', label="Distribution of Final Values")
        ax.axvline(invested, color='red', linestyle='--', linewidth=2, label=f'Invested ({invested:,.0f} €)')
        ...
        ax.axvspan(p25, p75, color='orange', alpha=0.2, label=f'25-75% Percentile ({p25:,.0f} € - {p75:,.0f} €)')
        ax.set_title(f"Monte Carlo Simulation over {years} Years", fontsize=14)
        ax.set_xlabel("Final Value in EUR", fontsize=10)
